...
return NProgress;});;frappe._messages={};frappe._=function(txt,replace){if(!txt)return txt;if(typeof(txt)!="string")return txt;ret=frappe._messages[txt.replace(/\n/g,"")]||txt;if(replace&&typeof(replace)==="object"){ret=$.format(ret,replace);}
return ret;};window.__=frappe._;function prettyDate(time){if(moment){if(frappe.boot){var user_timezone=frappe.boot.user.time_zone;var system_timezone=sys_defaults.time_zone;var zones=(frappe.boot.timezone_info||{}).zones||{};}
...
setInterval(function(){frappe.datetime.refresh_when()},60000);;
/*
...
frappe.provide("website");$.extend(frappe,{_assets_loaded:[],require:function(url){if(frappe._assets_loaded.indexOf(url)!==-1)return;$.ajax({url:url,async:false,dataType:"text",success:function(data){if(url.split(".").splice(-1)=="js"){var el=document.createElement('script');}else{var el=document.createElement('style');}
el.appendChild(document.createTextNode(data));document.getElementsByTagName('head')[0].appendChild(el);frappe._assets_loaded.push(url);}});},hide_message:function(text){$('.message-overlay').remove();},call:function(opts){frappe.prepare_call(opts);return $.ajax({type:opts.type||"POST",url:"/",data:opts.args,dataType:"json",statusCode:{404:function(xhr){frappe.msgprint(__("Not found"));},403:function(xhr){frappe.msgprint(__("Not permitted"));},200:function(data,xhr){if(opts.callback)
opts.callback(data);}}}).always(function(data){if(data.responseText){data=JSON.parse(data.responseText);}
frappe.process_response(opts,data);});},prepare_call:function(opts){if(opts.btn){$(opts.btn).prop("disabled",true);}
if(opts.msg){$(opts.msg).toggle(false);}
if(!opts.args)opts.args={};if(!opts.args._type){opts.args._type=opts.type||"GET";}
if(opts.method){opts.args.cmd=opts.method;}
$.each(opts.args,function(key,val){if(typeof val!="string"){opts.args[key]=JSON.stringify(val);}});if(!opts.no_spinner){}},process_response:function(opts,data){if(opts.btn){$(opts.btn).prop("disabled",false);}
if(data._server_messages){var server_messages=(JSON.parse(data._server_messages||'[]')).join("<br>");if(opts.error_msg){$(opts.error_msg).html(server_messages).toggle(true);}else{frappe.msgprint(server_messages);}}
if(data.exc){if(opts.btn){$(opts.btn).addClass("btn-danger");setTimeout(function(){$(opts.btn).removeClass("btn-danger");},1000);}
try{var err=JSON.parse(data.exc);if($.isArray(err)){err=err.join("\n");}
console.error?console.error(err):console.log(err);}catch(e){console.log(data.exc);}}else{if(opts.btn){$(opts.btn).addClass("btn-success");setTimeout(function(){$(opts.btn).removeClass("btn-success");},1000);}}
if(opts.msg&&data.message){$(opts.msg).html(data.message).toggle(true);}},show_message:function(text,icon){if(!icon)icon="icon-refresh icon-spin";frappe.hide_message();$('<div class="message-overlay"></div>').html('<div class="content"><i class="'+icon+' text-muted"></i><br>'
+text+'</div>').appendTo(document.body);},hide_message:function(text){$('.message-overlay').remove();},get_sid:function(){var sid=getCookie("sid");return sid&&sid!=="Guest";},get_modal:function(title,body_html){var modal=$('<div class="modal" style="overflow: auto;" tabindex="-1">\
   ...
   </div>').appendTo(document.body);return modal;},msgprint:function(html,title){if(html.substr(0,1)==="[")html=JSON.parse(html);if($.isArray(html)){html=html.join("<hr>")}
return frappe.get_modal(title||"Message",html).modal("show");},send_message:function(opts,btn){return frappe.call({type:"POST",method:"frappe.templates.pages.contact.send_message",btn:btn,args:opts,callback:opts.callback});},has_permission:function(doctype,docname,perm_type,callback){return frappe.call({method:"frappe.client.has_permission",no_spinner:true,args:{doctype:doctype,docname:docname,perm_type:perm_type},callback:function(r){if(!r.exc&&r.message.has_permission){if(callback){return callback(r);}}}});},render_user:function(){var sid=frappe.get_cookie("sid");if(sid&&sid!=="Guest"){$(".btn-login-area").toggle(false);$(".logged-in").toggle(true);$(".full-name").html(frappe.get_cookie("full_name"));$(".user-picture").attr("src",frappe.get_cookie("user_image"));}},setup_push_state:function(){if(frappe.supports_pjax()){window.initial_href=window.location.href
$(document).on("click","#wrap a",frappe.handle_click);$(window).on("popstate",function(event){if(location.hash&&(!window.previous_href||window.previous_href.replace(location.hash,'')===location.href.replace(location.hash,'')))
return;if(window.initial_href==location.href&&window.previous_href==undefined){window.history.replaceState({"reload":true},window.document.title,location.href);return;}
window.previous_href=location.href;var state=event.originalEvent.state;if(!state){window.location.reload();return;}
frappe.render_json(state);});}},handle_click:function(event){var link=event.currentTarget
if(link.tagName.toUpperCase()!=='A')
throw"using pjax requires an anchor element";if(event.which>1||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)
return;if(link.getAttribute("target"))
return;if(location.protocol!==link.protocol||location.hostname!==link.hostname)
return;if(link.hash&&link.href.replace(link.hash,'')===location.href.replace(location.hash,''))
return;if(link.href===location.href+'#')
return;if(link.href.indexOf("cmd=")!==-1||link.hasAttribute("no-pjax"))
return;var last_part=(link.href.split("/").slice(-1)[0]||"");if(last_part.indexOf(".")!==-1&&(last_part.indexOf(".htm")===-1))
return;event.preventDefault()
frappe.load_via_ajax(link.href);},load_via_ajax:function(href){window.previous_href=href;history.pushState(null,null,href);var _render=function(data){try{history.replaceState(data,data.title,href);}catch(e){history.replaceState(null,data.title,href);}
scroll(0,0);frappe.render_json(data);};$.ajax({url:href,cache:false,statusCode:{200:_render,404:function(xhr){_render(xhr.responseJSON);}}}).fail(function(xhr,status,error){window.location.reload();});},render_json:function(data){if(data.reload){window.location.reload();return;}
$('[data-html-block]').each(function(i,section){var $section=$(section);var stype=$section.attr("data-html-block");if(stype==="meta_block")return;var block_data=data[stype]||"";if(stype==="script"){$section.remove();$("<script data-html-block='script'></script>").html(block_data).appendTo("body");}else if(stype==="script_lib"){if(!$("[data-block-html='script_lib'][data-path='"+data.path+"']").length){$("<script data-block-html='script_lib' data-path='"+data.path+"'></script>").html(data.script_lib).appendTo("body");}}else{$section.html(block_data);}});if(data.title)$("title").html(data.title);$('[data-html-block="meta_block"]').remove();if(data.meta_block){$("head").append(data.meta_block);}
$(".page-container").attr("id","page-"+data.path);$(".page-header-right").html("");window.ga&&ga('send','pageview',location.pathname);$(document).trigger("page-change");},supports_pjax:function(){return(window.history&&window.history.pushState&&window.history.replaceState&&!navigator.userAgent.match(/((iPod|iPhone|iPad).+\bOS\s+[1-4]|WebApps\/.+CFNetwork)/))},get_pathname:function(){return location.pathname;},page_ready_events:{},ready:function(fn){frappe.page_ready_events[frappe.get_pathname()]=fn;},trigger_ready:function(){var ready=frappe.page_ready_events[frappe.get_pathname()];ready&&ready();},make_navbar_active:function(){var pathname=window.location.pathname;$(".navbar a.active").removeClass("active");$(".navbar a").each(function(){var href=$(this).attr("href");if(href===pathname){$(this).addClass("active");return false;}})},toggle_template_blocks:function(){$(".page-header").toggleClass("hidden",!!!$("[data-html-block='header']").text().trim());$(".page-footer").toggleClass("hidden",!!!$(".page-footer").text().trim());$("[data-html-block='breadcrumbs'] .breadcrumb").toggleClass("hidden",!$("[data-html-block='breadcrumbs']").text().trim()||$("[data-html-block='breadcrumbs']").text().trim()==$("[data-html-block='header']").text().trim());var sidebar_has_content=!!$("[data-html-block='sidebar']").html().trim();$(".page-sidebar, .toggle-sidebar").toggleClass("hidden",!sidebar_has_content);$(".page-sidebar").toggleClass("col-sm-push-9",sidebar_has_content);$(".page-content").toggleClass("col-sm-12",!sidebar_has_content);$(".page-content").toggleClass("col-sm-9 col-sm-pull-3",sidebar_has_content);if(!$(".page-sidebar").hasClass("hidden-xs")){$(".toggle-sidebar").trigger("click");}}});function valid_email(id){return(id.toLowerCase().search("[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")==-1)?0:1;}
var validate_email=valid_email;function get_url_arg(name){name=name.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");var regexS="[\\?&]"+name+"=([^&#]*)";var regex=new RegExp(regexS);var results=regex.exec(window.location.href);if(results==null)
return"";else
return decodeURIComponent(results[1]);}
function make_query_string(obj){var query_params=[];$.each(obj,function(k,v){query_params.push(encodeURIComponent(k)+"="+encodeURIComponent(v));});return"?"+query_params.join("&");}
function repl(s,dict){if(s==null)return'';for(key in dict){s=s.split("%("+key+")s").join(dict[key]);}
return s;}
function replace_all(s,t1,t2){return s.split(t1).join(t2);}
function getCookie(name){return getCookies()[name];}
frappe.get_cookie=getCookie;function getCookies(){var c=document.cookie,v=0,cookies={};if(document.cookie.match(/^\s*\$Version=(?:"1"|1);\s*(.*)/)){c=RegExp.$1;v=1;}
if(v===0){c.split(/[,;]/).map(function(cookie){var parts=cookie.split(/=/,2),name=decodeURIComponent(parts[0].trimLeft()),value=parts.length>1?decodeURIComponent(parts[1].trimRight()):null;if(value&&value.charAt(0)==='"'){value=value.substr(1,value.length-2);}
cookies[name]=value;});}else{c.match(/(?:^|\s+)([!#$%&'*+\-.0-9A-Z^`a-z|~]+)=([!#$%&'*+\-.0-9A-Z^`a-z|~]*|"(?:[\x20-\x7E\x80\xFF]|\\[\x00-\x7F])*")(?=\s*[,;]|$)/g).map(function($0,$1){var name=$0,value=$1.charAt(0)==='"'?$1.substr(1,-1).replace(/\\(.)/g,"$1"):$1;cookies[name]=value;});}
return cookies;}
...
return a;};}
function remove_script_and_style(txt){return(!txt||(txt.indexOf("<script>")===-1&&txt.indexOf("<style>")===-1))?txt:$("<div></div>").html(txt).find("script,noscript,style,title,meta").remove().end().html();}
function is_html(txt){if(txt.indexOf("<br>")==-1&&txt.indexOf("<p")==-1&&txt.indexOf("<img")==-1&&txt.indexOf("<div")==-1){return false;}
return true;}
function ask_to_login(){if(!window.full_name){if(localStorage){localStorage.setItem("last_visited",window.location.href.replace(window.location.origin,""));}
window.location.href="login";}}
$(document).ready(function(){window.full_name=getCookie("full_name");window.logged_in=getCookie("sid")&&getCookie("sid")!=="Guest";$("#website-login").toggleClass("hide",logged_in?true:false);$("#website-post-login").toggleClass("hide",logged_in?false:true);$(".toggle-sidebar").on("click",function(){$(".page-sidebar").toggleClass("hidden-xs");$(".toggle-sidebar i").toggleClass("icon-rotate-180");});if(getCookie("system_user")==="yes"){$("#website-post-login .dropdown-menu").append('<li class="divider"></li>\
   <li><a href="/desk" no-pjax><i class="icon-fixed-width icon-th-large"></i> Switch To Desk</a></li>');}
frappe.render_user();frappe.setup_push_state()
$(document).trigger("page-change");});$(document).on("page-change",function(){$(document).trigger("apply_permissions");frappe.datetime.refresh_when();frappe.toggle_template_blocks();frappe.trigger_ready();frappe.make_navbar_active();});;frappe.provide("website");$.extend(website,{toggle_permitted:function(){if(website.access){$('li[data-view="add"]').toggleClass("hide",!website.access.write);$('li[data-view="settings"]').toggleClass("hide",!website.access.admin);$('li[data-view="edit"]').toggleClass("hide",website.view!=="edit");if(!website.access.write){var sid=frappe.get_cookie("sid");if(!sid||sid==="Guest"){$(".post-list-help").html("Please login to post");}else{$(".post-list-help").html("You do not have permission to post");}}else{$(".post-list-help").html("");}}},setup_pagination:function(){var $btn=$(".btn-more").removeClass("hide").on("click",function(){frappe.call({btn:$btn,type:"GET",args:{cmd:"frappe.templates.website_group.forum.get_post_list_html",limit_start:$(".post").length,limit_length:20,group:website.group,view:website.view,pathname:website.pathname},callback:function(data){$(".post-list").append(data.message);$btn.toggleClass("hide",!(data.message&&data.message.length===opts.args.limit_length));website.toggle_upvote();}})});},bind_add_post:function(){$(".btn-post-add").on("click",website.add_post);$pic_input=$(".control-post-add-picture").on("change",website.add_picture);$(".btn-post-add-picture").on("click",function(){$pic_input.click();});},add_post:function(){if(website.post){frappe.msgprint(__("Post already exists. Cannot add again!"));return;}
website._update_post(this,"frappe.templates.website_group.post.add_post");},bind_save_post:function(){$(".btn-post-add").addClass("hide");$(".btn-post-save").removeClass("hide").on("click",website.save_post);$(".post-picture").toggleClass("hide",!$(".post-picture").attr("src"));},save_post:function(){if(!website.post){frappe.msgprint(__("Post does not exist. Please add post!"));return;}
website._update_post(this,"frappe.templates.website_group.post.save_post");},_update_post:function(btn,cmd){var values=website.get_editor_values();if(!values){return;}
frappe.call({btn:btn,type:"POST",args:$.extend({cmd:cmd,group:website.group,post:website.post||undefined,},values),callback:function(data){var url=window.location.pathname+"?view=post&name="+data.message;window.location.href=url;}});},get_editor_values:function(){var values={};$.each($('.post-editor [data-fieldname]'),function(i,ele){var $ele=$(ele);values[$ele.attr("data-fieldname")]=$ele.val();});values.parent_post=$(".post-editor").attr("data-parent-post");values.picture_name=$(".control-post-add-picture").val()||null;var dataurl=$(".post-picture img").attr("src");values.picture=dataurl?dataurl.split(",")[1]:""
if(!values.parent_post&&!values.title){frappe.msgprint(__("Please enter title!"));return;}else if(!values.content){frappe.msgprint(__("Please enter some text!"));return;}else if($('.post-editor [data-fieldname="event_datetime"]').length&&!values.event_datetime){frappe.msgprint(__("Please enter Event's Date and Time!"));return;}
values.content=website.process_external_links(values.content);return values;},process_external_links:function(content){return content.replace(/([^\s]*)(http|https|ftp):\/\/[^\s\[\]\(\)]+/g,function(match,p1){if(p1)return match;return"["+match+"]("+match+")";});},add_picture:function(){if(this.type==='file'&&this.files&&this.files.length>0){$.each(this.files,function(idx,fileobj){if(/^image\//.test(fileobj.type)){$.canvasResize(fileobj,{width:500,height:0,crop:false,quality:80,callback:function(data,width,height){$(".post-picture").removeClass("hide").find("img").attr("src",data);}});}});}
return false;},setup_tasks_editor:function(){var $post_editor=$(".post-editor");var $control_assign=$post_editor.find('.control-assign');var bind_close=function(){var close=$post_editor.find("a.close")
if(close.length){close.on("click",function(){$post_editor.find(".assigned-to").addClass("hide");$post_editor.find(".assigned-user").html("");$post_editor.find('[data-fieldname="assigned_to"]').val(null);$control_assign.val(null);});}}
if($control_assign.length){website.setup_autosuggest({$control:$control_assign,select:function(value,item){var $assigned_to=$post_editor.find(".assigned-to").removeClass("hide");$assigned_to.find(".assigned-user").html(item.user_html);$post_editor.find('[data-fieldname="assigned_to"]').val(value);bind_close();},method:"frappe.templates.website_group.post.suggest_user"});bind_close();}},setup_event_editor:function(){var $post_editor=$(".post-editor");var $control_event=$post_editor.find('.control-event').empty();var $event_field=$post_editor.find('[data-fieldname="event_datetime"]');var set_event=function($control){var datetime=website.datetimepicker.obj_to_str($control_event.datepicker("getDate"));if($event_field.val()!==datetime){$event_field.val(datetime);}};website.setup_datepicker({$control:$control_event,onClose:function(){set_event($control_event)}});if($event_field.val()){$control_event.val(website.datetimepicker.format_datetime($event_field.val()));}},format_event_timestamps:function(){var get_day=function(num){return["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"][num];}
var get_month=function(num){return["January","February","March","April","May","June","July","August","September","October","November","December"][num-1];}
var format=function(datetime){if(!datetime)return"";var date=datetime.split(" ")[0].split("-");var time=datetime.split(" ")[1].split(":");var tt="am";if(time[0]>=12){time[0]=parseInt(time[0])-12;tt="pm";}
if(!parseInt(time[0])){time[0]=12;}
var hhmm=[time[0],time[1]].join(":")
var dateobj=new Date(date[0],date[1],date[2])
return repl("%(day)s, %(date)s %(month)s, %(year)s %(time)s",{day:get_day(dateobj.getDay()),date:date[2],month:get_month(dateobj.getMonth()),year:date[0],time:hhmm+" "+tt})}
$(".event-timestamp").each(function(){$(this).html(format($(this).attr("data-timestamp")));})},toggle_earlier_replies:function(){var $earlier_replies=$(".child-post").slice(0,$(".child-post").length-2);var $btn=$(".btn-earlier-replies").on("click",function(){if($earlier_replies.hasClass("hide")){$earlier_replies.removeClass("hide");$(".btn-earlier-label").html("Hide");}else{$earlier_replies.addClass("hide");$(".btn-earlier-label").html("Show");}});if($earlier_replies.length){$btn.toggleClass("hide",false).click();}},toggle_edit:function(only_owner){if(only_owner){var user=frappe.get_cookie("user_id");$(".edit-post").each(function(){$(this).toggleClass("hide",!(website.access.write&&$(this).attr("data-owner")===user));});}else{$(".edit-post").toggleClass("hide",!website.access.write);}},toggle_upvote:function(){if(!website.access.read||!website.upvote){$(".upvote").remove();}},toggle_post_editor:function(){$(".post-editor").toggleClass("hide",!website.access.write);},setup_upvote:function(){if(window.website.setup_upvote_done)return;window.website.setup_upvote_done=true;$(document).on("click",".post-list .upvote a, .parent-post .upvote a",function(){console.log("clicked");var sid=frappe.get_cookie("sid");if(!sid||sid==="Guest"){frappe.msgprint(__("Please login to Upvote!"));return;}
var $post=$(this).parents(".post");var post=$post.attr("data-name");var $btn=$(this).prop("disabled",true);$.ajax({url:"/",type:"POST",data:{cmd:"frappe.website.doctype.user_vote.user_vote.set_vote",ref_doctype:"Post",ref_name:post},statusCode:{200:function(data){if(data.exc){console.log(data.exc);}else{var text_class=data.message==="ok"?"text-success":"text-danger";if(data.message==="ok"){var count=parseInt($post.find(".upvote-count").text());$post.find(".upvote-count").text(count+1).removeClass("hide");}
$btn.addClass(text_class);setTimeout(function(){$btn.removeClass(text_class);},2000);}}}}).always(function(){$btn.prop("disabled",false);});return false;});},setup_autosuggest:function(opts){if(opts.$control.hasClass("ui-autocomplete-input"))return;frappe.require("/assets/frappe/js/lib/jquery/jquery.ui.min.js");frappe.require("/assets/frappe/js/lib/jquery/bootstrap_theme/jquery-ui.selected.css");var $user_suggest=opts.$control.autocomplete({source:function(request,response){$.ajax({url:"/",data:{cmd:opts.method,term:request.term,group:website.group},success:function(data){if(data.exc){console.log(data.exc);}else{response(data.message);}}});},select:function(event,ui){opts.$control.val("");opts.select(ui.item.user,ui.item);}});$user_suggest.data("ui-autocomplete")._renderItem=function(ul,item){return $("<li>").html("<a style='padding: 5px;'>"+item.user_html+"</a>").css("padding","5px").appendTo(ul);};return opts.$control},setup_datepicker:function(opts){if(opts.$control.hasClass("hasDatetimepicker"))return;frappe.require("/assets/frappe/js/lib/jquery/jquery.ui.min.js");frappe.require("/assets/frappe/js/lib/jquery/bootstrap_theme/jquery-ui.selected.css");frappe.require("/assets/frappe/js/lib/jquery/jquery.ui.slider.min.js");frappe.require("/assets/frappe/js/lib/jquery/jquery.ui.sliderAccess.js");frappe.require("/assets/frappe/js/lib/jquery/jquery.ui.timepicker-addon.css");frappe.require("/assets/frappe/js/lib/jquery/jquery.ui.timepicker-addon.js");opts.$control.datetimepicker({timeFormat:"hh:mm tt",dateFormat:'DD, d MM, yy',changeYear:true,yearRange:"-70Y:+10Y",stepMinute:5,hour:10,onClose:opts.onClose});website.setup_datetime_functions();return opts.$control;},setup_datetime_functions:function(){frappe.provide("website.datetimepicker");website.datetimepicker.str_to_obj=function(datetime_str){return $.datepicker.parseDateTime("yy-mm-dd","HH:mm:ss",datetime_str);};website.datetimepicker.obj_to_str=function(datetime){if(!datetime){return"";}
var date_str=$.datepicker.formatDate("yy-mm-dd",datetime)
var time_str=$.datepicker.formatTime("HH:mm:ss",{hour:datetime.getHours(),minute:datetime.getMinutes(),second:datetime.getSeconds()})
return date_str+" "+time_str;};website.datetimepicker.format_datetime=function(datetime){if(typeof(datetime)==="string"){datetime=website.datetimepicker.str_to_obj(datetime);}
var date_str=$.datepicker.formatDate("DD, d MM, yy",datetime)
var time_str=$.datepicker.formatTime("hh:mm tt",{hour:datetime.getHours(),minute:datetime.getMinutes(),second:datetime.getSeconds()})
return date_str+" "+time_str;}},setup_settings:function(){website.setup_autosuggest({$control:$(".add-user-control"),select:function(value){website.add_sitemap_permission(value);},method:"frappe.templates.website_group.settings.suggest_user"});$(".permission-editor-area").on("click",".sitemap-permission [type='checkbox']",website.update_permission);$(".permission-editor-area").find(".btn-add-group").on("click",website.add_group);$(".btn-settings").parent().addClass("active");var control_public_read=$(".control-add-group-public_read").click(function(){if(!$(this).prop("checked")){$(".control-add-group-public_write").prop("checked",false).prop("disabled",true);}else{$(".control-add-group-public_write").prop("disabled",false);}}).trigger("click").trigger("click");},add_group:function(){var $control=$(".control-add-group"),$btn=$(".btn-add-group");if($control.val()){$btn.prop("disabled",true);$.ajax({url:"/",type:"POST",data:{cmd:"frappe.templates.website_group.settings.add_website_group",group:website.group,new_group:$control.val(),group_type:$(".control-add-group-type").val(),public_read:$(".control-add-group-public_read").is(":checked")?1:0,public_write:$(".control-add-group-public_write").is(":checked")?1:0},statusCode:{403:function(){frappe.msgprint(__("Name not permitted"));},200:function(data){if(data.exc){console.log(data.exc);if(data._server_messages)frappe.msgprint(data._server_messages);}else{frappe.msgprint(__("Group Added, refreshing..."));setTimeout(function(){window.location.reload();},1000)}}}}).always(function(){$btn.prop("disabled",false);$control.val("");})}},update_permission:function(){var $chk=$(this);var $tr=$chk.parents("tr:first");$chk.prop("disabled",true);$.ajax({url:"/",type:"POST",data:{cmd:"frappe.templates.website_group.settings.update_permission",user:$tr.attr("data-user"),perm:$chk.attr("data-perm"),value:$chk.prop("checked")?"1":"0",group:website.group},statusCode:{403:function(){frappe.msgprint(__("Not allowed"));},200:function(data){$chk.prop("disabled",false);if(data.exc){$chk.prop("checked",!$chk.prop("checked"));console.log(data.exc);}else{if(!$tr.find(":checked").length)$tr.remove();}}},});},add_sitemap_permission:function(user){$.ajax({url:"/",type:"POST",data:{cmd:"frappe.templates.website_group.settings.add_sitemap_permission",user:user,group:website.group},success:function(data){$(".add-user-control").val("");if(data.exc){console.log(data.exc);}else{$(data.message).prependTo($(".permission-editor tbody"));}}});},update_group_description:function(){$(".btn-update-description").prop("disabled",true);$.ajax({url:"/",type:"POST",data:{cmd:"frappe.templates.website_group.settings.update_description",description:$(".control-description").val()||"",group:website.group},success:function(data){window.location.reload();}}).always(function(){$(".btn-update-description").prop("disabled",false);});}});$(document).on("apply_permissions",function(){website.toggle_permitted();});;